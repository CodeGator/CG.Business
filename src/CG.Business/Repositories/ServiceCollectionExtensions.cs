...
namespace Microsoft.Extensions.DependencyInjection
{
    ...
    public static partial class ServiceCollectionExtensions
    {
            ...
            )
        {
            ...
            // Are we pointed to a repository section?
            if (false == path.EndsWith("Repository"))
            {
                // Point to the proper section.
                configuration = configuration.GetSection("Repository");

                // Get the new path.
                path = configuration.GetPath();
            }

            // Now we should be pointed to the repository section, but, just in case, 
            ...
            if (false == path.EndsWith("Repository"))
            {
                ...
                throw new ConfigurationException(
                    message: string.Format(
                        ...
                        path,
                        $"{configuration.GetPath()}:Repository"
                        )
                    );
            }
        ...
        /// compatible section. It then uses that information to dynamically locate,
        /// and then invoke, an extension method that registers concrete options 
        /// and repository types. This variant is intended to work with custom 
        ...
        /// extension method(s) needed to register the repository types.  
        /// </remarks>
        public static IServiceCollection AddRepositories<TModel, TKey>(
            this IServiceCollection serviceCollection,
            IConfiguration configuration,
            string assemblyWhiteList = "",
            string assemblyBlackList = "Microsoft*, System*, mscorlib, netstandard"
              ...
              where TKey : new()
        {
            ...
            // Look for specified extension method.
            var methods = AppDomain.CurrentDomain.ExtensionMethods<TModel, TKey>(
                typeof(IServiceCollection),
                methodName,
                new Type[] { typeof(IConfiguration) },
                assemblyWhiteList,
